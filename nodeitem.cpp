...
NodeItem::NodeItem(int id, QGraphicsItem *parent)
    : QGraphicsEllipseItem(-20, -20, 40, 40, parent), id(id) {
    ...
    setFlag(
        QGraphicsItem::ItemSendsGeometryChanges);  // узел отправляет сообщение
    ...
    label->setHtml(
        QString("<center><span style='color: black;'>%1</span></center>")
            .arg(id));
...
void NodeItem::addEdge(EdgeItem *edge) { edges.append(edge); }

void NodeItem::removeEdge(EdgeItem *edge) { edges.removeOne(edge); }
